...
function App() {
  ...
  //   setTick(prev => !prev);
  //   try {
  //     const response = await fetch(`http://localhost:8081/api/deyPlayPostTick/${id}`, {
  //       method: "PUT",
  //       headers: {
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      try {
        const response = await fetch("http://localhost:8081/api/getthings");
  ...
  const handleAddTodoSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://localhost:8081/api/acceptthings", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`http://localhost:8081/api/acceptthings/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-xl mx-auto p-4 bg-gray-500">
      ...
      <form onSubmit={handleAddTodoSubmit} className="mb-4">
        <div className="flex">
          ...
          <button
            type="submit"
            className="bg-white text-black px-4 py-2 rounded-r hover:text-white hover:bg-blue-600"
          >
            Add +
          </button>
        </div>
