...
function App() {
  ...
  async function handleAddTodoSubmit(event) {
    ...
    try {
      const response = await fetch("http://localhost:5173/api/todos2", {
        method: "POST",
        headers: {
  ...
  async function deleteItem(id) {
    try {
      const response = await fetch(`http://localhost:5173/api/todos2/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      <h1 className="text-green-500 font-semibold text-4xl mt-4 mb-6">
      JA Samuel's toDo List
      </h1>
